    ...
    for (let key in schema) {
      tempFields[key] = {
        ...
        step: schema[key].step
          ? schema[key].step
          : schema[key].depend
          ? schema[options[key].parentField].step
          : 1,
    ...
    if (props.schema.wizard) {
      return (
        <MultipleForm
          ...
          step={setStep}
          button={{
            next: props.schema.nextButtonText,
            previous: props.schema.prevButtonText,
            submit: props.schema.submitButtonText,
          }}
  ...
  return (
    <>
      <div
        ...
        <h1 style={{ fontFamily: "Nunito Sans", margin: "20px 0px" }}>
          {props.schema?.title}
        </h1>
        <h2
          style={{
        ...
        <Progress
          height={props.schema.progressBarHeight}
          step={step}
          stepLength={Object.keys(wizardStepSet).length}
          color={props.schema.progressBarColor}
          text={props.schema.progressBarText}
          textColor={props.schema.progressBarTextColor}
          ProgressBar={props.schema.ProgressBar}
          subProgressBar={props.schema.subProgressBar}
        />
        <form
          onSubmit={(e) => {
            e.preventDefault();
